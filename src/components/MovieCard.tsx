...
export function MovieCard({ movie }: MovieCardProps) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 p-4">
          ...
          <button
            ...
            className="mt-3 w-full rounded-full border border-white/30 px-3 py-1.5 text-sm font-medium uppercase tracking-wide text-white opacity-100 transition-all duration-300 hover:border-white focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-900 sm:opacity-0 sm:group-hover:opacity-100"
          >
            Details
          </button>
        </div>
      ...
      {isDetailOpen && (
        <MovieDetailModal movie={movie} isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} />
      )}
    </>
  );
}
